...
void Agent::ChangeState(FSMState * new_state)
{
...
void Agent::PrintStats()
{
	...
	if (clock() > lastTime + refreshRate) {
		lastTime = clock();

		system("cls");

		std::cout << "---------------" << std::endl;
		std::cout << "Current Location: " << currState->GetName() << std::endl;
		std::cout << "Money:" << (int)money << "(Max: 1000)" <<std::endl;
		std::cout << "Gold:" << (int)gold << "(Max: 100)"<< std::endl;
		std::cout << "Thirst:" << (int)thirst <<"(Max: 100)"<< std::endl;
		std::cout << "---------------" << std::endl;
	}
